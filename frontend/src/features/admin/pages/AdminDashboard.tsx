    ...
    return (
        <div className="p-6 grid grid-cols-12 gap-6 relative min-h-screen">
            ...
            <div className="col-span-12 lg:col-span-8 xl:col-span-9 flex flex-col gap-6">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <button className="px-3 py-1.5 text-xs font-medium text-slate-300 bg-surface-dark border border-slate-800 rounded hover:border-primary transition-colors flex items-center gap-1">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                    {pendingIncidents.length === 0 ? (
                        <div className="col-span-full p-8 text-center bg-surface-dark rounded-lg border border-slate-800 text-slate-500">
                        ...
                        pendingIncidents.map((incident) => (
                            <div key={incident.id} className="group relative bg-surface-dark border-l-4 border-l-primary border-y border-r border-slate-800 rounded-r-lg shadow-sm hover:shadow-md transition-all p-5">
                                ...
                                <div className="mb-4 pr-12">
                                    <h3 className="text-lg font-bold text-white mb-1">{incident.type}</h3>
                                ...
                                <div className="flex flex-wrap items-end justify-between gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <div className="w-16 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                                                    <div className="h-full bg-primary" style={{ width: `${incident.confidence * 100}%` }}></div>
                                                </div>
                                                <span className="font-bold text-primary">{Math.round(incident.confidence * 100)}%</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                        <Link
                                            ...
                                            className="px-4 py-2 bg-primary hover:bg-primary-dark text-white text-sm font-medium rounded shadow-lg shadow-primary/20 transition-all flex items-center gap-2"
                                        >
                ...
                <div className="mt-auto">
                    <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4">Recent Actions</h2>
                    <div className="bg-surface-dark border border-slate-800 rounded-lg p-4">
                        <div className="relative pl-6 border-l border-slate-800 space-y-6">
                            {recentActions.length === 0 ? <p className="text-sm text-slate-500">No recent actions.</p> : recentActions.map((action, idx) => (
                                <div key={idx} className="relative">
                                    <span className={clsx(
                                        "absolute -left-[29px] top-1 h-3 w-3 rounded-full ring-4 ring-surface-dark",
            ...
            <div className="col-span-12 lg:col-span-4 xl:col-span-3 flex flex-col h-full gap-6">
                ...
                <div className="flex flex-col h-full bg-surface-dark border border-slate-800 rounded-lg overflow-hidden">
                    <div className="p-4 border-b border-slate-800 bg-background-dark flex justify-between items-center">
                        <h2 className="text-sm font-bold text-white uppercase tracking-wider">Priority Queue</h2>
                        <span className="bg-primary/10 text-primary text-xs font-bold px-2 py-1 rounded">{pendingIncidents.length} Items</span>
                    </div>
                    <div className="flex-1 overflow-y-auto">
                        {pendingIncidents.map((incident) => (
                            <div key={incident.id} className="p-4 border-b border-slate-800 hover:bg-slate-800/50 transition-colors cursor-pointer group">
                ...
                <div className="bg-primary/10 border border-primary/20 rounded-lg p-4">
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between text-xs">
                            ...
                            <span className="text-white font-mono">{stats.avgResponseTime}</span>
                        </div>
